      ...
      <div
        ...
        <button
          ...
          {!isFilterActive && (
            <LucideChevronRight
              size={18}
              className={`transform transition-transform duration-150 ${isExpanded ? 'rotate-90' : 'rotate-0'}`}
            />
          )}
        </button>
        {/* Show 'X' button when filter is active */}
        {filteredCountryStore.selectedRegion !== 'All' && (
          <button type='button' className='absolute right-0 inset-y-0 flex items-center pr-3' onClick={clearFilter}>
      ...
      {isExpanded && (
        <div className='absolute mt-2 w-full bg-white rounded-md shadow-lg z-10 dark:bg-gray-500 dark:border-gray-500'>
          {[...filteredCountryStore.uniqueRegions].map((region) => (
            <button
              key={region}
              className='block w-full px-4 py-2 text-left rounded-md hover:bg-blue-50 focus:outline-none dark:bg-gray-500 dark:border-gray-500 dark:hover:bg-gray-900'
